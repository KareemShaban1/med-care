...
<div class="container mx-auto px-4 py-8">
    <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
        ...
        <div class="flex flex-col space-y-4">
            ...
            <p class="text-sm text-gray-500">{{ __('Category') }}: {{ $product->category->name ?? 'Uncategorized' }}</p>

            {{-- Prices --}}
            @if($product->old_price)
            <div class="flex items-center space-x-3 gap-3">
                <span class="text-xl font-semibold text-green-600">{{ __('EGP') }} {{ number_format($product->price,2) }}</span>
                <span class="text-base line-through text-red-500">{{ __('EGP') }} {{ number_format($product->old_price,2) }}</span>
            </div>
            @else
            <span class="text-xl font-semibold text-gray-700">{{ __('EGP') }} {{ number_format($product->price,2) }}</span>
            @endif
            ...
            <form method="POST" action="{{ route('cart.add', $product->id) }}" class="space-y-3">
                @csrf
                <div class="flex items-center space-x-3 gap-3">
                    <label for="quantity" class="text-gray-700 font-medium">{{ __('Quantity') }}</label>
    ...
    {{-- Related Products --}}
    <div class="mt-12">
        <h2 class="text-xl font-bold mb-4 text-gray-800">{{ __('Related Products') }}</h2>
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
            ...
            <div
                ...
                <div class="relative">
                    ...
                    @else
                    <div class="bg-gray-100 flex items-center justify-center h-40">{{ __('No Image') }}</div>
                    @endif
                </div>
                <div class="p-4">
                    <h6 class="font-semibold text-gray-800 truncate">{{ $related->name }}</h6>
                    <p class="text-sm text-gray-500">{{ __('Category') }}: {{ $related->category->name }}</p>
                    @if($related->old_price)
                    <div class="flex items-center space-x-3 gap-2">
                        <span class="text-xl font-semibold text-green-600">{{ __('EGP') }} {{ number_format($related->price,2) }}</span>
                        <span class="text-base line-through text-red-500">{{ __('EGP') }} {{ number_format($related->old_price,2) }}</span>
                    </div>
                    @else
                    <span class="text-xl font-semibold text-gray-700">{{ __('EGP') }} {{ number_format($related->price,2) }}</span>
                    @endif

                    {{-- Add to Cart --}}
                    <form method="POST" action="{{ route('cart.add', $related->id) }}" class="mt-3">
                        @csrf
                        <input type="hidden" name="quantity" value="1">
                        <button
                            class="btn btn-sm w-full rounded-lg text-white py-2 transition {{ $related->stock <= 0 ? 'bg-red-600 hover:bg-red-700 cursor-not-allowed opacity-100' : 'bg-green-600 hover:bg-green-700' }}"
